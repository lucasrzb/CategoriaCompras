...
#FINALIZADO 05/04/2025 V2.0
#FIZ 2 FORMAS DE SISTEMAS DEPENDENDO DA CATEGORIA
...
while True: #lopp inicio
    ...
    print(f'{azul}Categoria Menu Inicial: frutas , liquidos, vegetais, carne, sanduiche, salgado{resetar}')
    ...
    if comida.lower() == 'frutas':
        ...
        if fruta.lower() == 'Abacaxi': #.lower não estava funcionando então fiz gambiarra
            ...
            if abacaxi == '0':
                ...
                elif saldo < calculo: #para caso não tenha dinheiro
        ...
        if liquidos.lower() == 'Leite':
            ...
            #NOVA CATEGORIA
    elif comida == 'Vegetais':
        print(f'{amarelo}Categoria Vegetal: brocolis, coentro, alface, rúcula{resetar}')
        tipodevegetal = str(input(f'{azul}Digite um vegetal{resetar}: '))
        if tipodevegetal == 'brocolis':
            quantidade = int(input(f'{amarelo}Deseja quantas {tipodevegetal}{resetar}?: '))
            valor = 2.50
            calculo = quantidade * valor
            brocolis  = input(f'''{amarelo}Deseja comprar {tipodevegetal} por R${calculo}{resetar}?
                                        [0] {verde}Sim{resetar}
                                        [1] {vermelho}Não{resetar}: ''')
            if brocolis == '0':
                if saldo >= calculo:
                    saldo -= calculo #CALCULO DE DESCONTO NO SALDO
                    print(f'{verde}Você comprou {tipodevegetal} por R${calculo}{resetar}')
                elif saldo < calculo: # SE FOR MENOR O SALDO ELE NÃO VAI ACEITAR
                    print(f'{vermelho}Você não tem dinheiro suficiente{resetar}')
            elif brocolis == '1':
                print(f'{vermelho}Você voltou ao menu{resetar}')
        elif tipodevegetal == 'coentro':
            quantidade = int(input(f'{azul}Deseja quantas {tipodevegetal}{resetar}?: '))
            valor =  4.99
            calculo = quantidade * valor
            coentro = input(f'''{amarelo}Deseja comprar {tipodevegetal} por R${calculo}{resetar}? 
                                        [0] {verde}Sim{resetar}
                                        [1] {vermelho}Não{resetar}: ''')
            if coentro == '0':
                if saldo >= calculo:
                ...
                print(f'{vermelho}Você voltou ao menu{resetar}')
        elif tipodevegetal == 'alface':
            quantidade = int(input(f'{amarelo}Deseja quantas {tipodevegetal}{resetar}?: '))
            valor = 1.99
            calculo = quantidade * valor
            alface = input(f'''{amarelo}Deseja comprar {tipodevegetal} por R${calculo}{resetar}?
                                    [0] {verde}Sim
                                    [1] {vermelho}Não{resetar}: ''')
            if alface == '0':
                if saldo >= calculo:
                ...
                print(f'{vermelho}Você voltou ao menu{resetar}')
        elif tipodevegetal == 'rúcula':
            quantidade = int(input(f'{amarelo}Quantos {tipodevegetal} deseja?{resetar}: '))
            valor = 1.99
            calculo = quantidade * valor
            rucula = input(f'''{amarelo}Deseja comprar {tipodevegetal} por R${calculo}{resetar}?
                          [0] {verde}Sim{resetar}
                          [1] {vermelho}Não{resetar}: ''')
            if rucula == '0':
                if saldo >= calculo:
        ...
        tipodesanduiche = str(input('Digite um tipo de sanduiche: '))
        if tipodesanduiche == 'X-Salada':
            print('Alface, Pão de hambuguer, tomate, alface')
            xsalada = input(f'''{amarelo}Deseja comprar X-Salada por R$14.00?{resetar} 
                          [0] {verde}Sim{resetar}
                          [1] {vermelho}Não{resetar}: ''')
            if xsalada == '0':
                item = 14.00
                if saldo >= item:
                    saldo -= item
                    print(f'{verde}Você comprou {tipodesanduiche} por R${item}{resetar}')
                elif saldo < item:
                    print(f'{vermelho}Você não tem dinheiro suficiente{resetar}')
            elif xsalada == '1':
                print(f'{vermelho}Você voltou ao menu{resetar}')
            ...
            xbacon = input(f'''{amarelo}Deseja comprar {tipodesanduiche} por R$23.00?{resetar}
                          [0] {verde}Sim{resetar} 
                          [1] {vermelho}Não{resetar}: ''')
            if xbacon == '0':
                item = 23.00
            ...
            if xespecial == '0':
                ...
                if saldo >= item:
                    saldo -= item
                    print(f'{amarelo}Você comprou {tipodesanduiche} por R${item}{resetar}')
                elif saldo < item:
                    print(f'{vermelho}Você não tem dinheiro suficiente{resetar}')
            elif xespecial == '1':
                print(f'{vermelho}Você voltou ao menu{resetar}')
            ...
            xtudo = input(f'''{amarelo}Deseja comprar {tipodesanduiche} por R$32.00?{resetar}.
                              [0] {verde}Sim{resetar}
                              [1] {vermelho}Não{resetar}: ''')
            if xtudo == '0':
                item = 32.00
            ...
            xmirage = input(f'''{amarelo}Deseja comprar {tipodesanduiche} por R$19.00?{resetar} 
                              [0] {verde}Sim{resetar} 
                              [1] {vermelho}Não{resetar}: ''')
            if xmirage == '0':
                item = 19.00
                if saldo >= item:
                    saldo -= item
                    print(f'{verde}Você comprou {tipodesanduiche} por {item}{resetar}')
                elif saldo < item:
                    print(f'{vermelho}Você não tem dinheiro suficiente{resetar}')
            elif xmirage == '1':
                print(f'{vermelho}Você voltou ao menu{resetar}')
        ...
        tipodesalgado = str(input(f'{azul}Digite o seu salgado preferido{resetar}: '))
        if tipodesalgado == 'americano':
            print('Queijo e persunto sempre será a melhor combinação')
            americano = input(f'''{amarelo}Deseja comprar {tipodesalgado} por R$3.5?{resetar} 
                              [0] {verde}Sim{resetar}
                              [1] {vermelho}Não?{resetar}: ''')
            if americano == '0':
                item = 3.50
                if saldo >= item:
                    saldo -= item
                    print(f'{verde}Você comprou {tipodesalgado} por R${item}{resetar}')
                elif saldo < item:
                    print(f'{vermelho}Você não tem dinheiro suficiente{resetar}')
                elif americano == '1':
                    print(f'{vermelho}Você voltou ao menu{resetar}')
            ...
            enroladinho = input(f'''{amarelo}Deseja comprar {tipodesalgado} por R$2.50?{resetar} 
                           [0] {verde}Sim{resetar}
                           [1] {vermelho}Não{resetar}: ''')
            if enroladinho == '0':
                item = 2.50
                if saldo >= item:
                    saldo -= item
                    print(f'{vermelho}Você comprou {tipodesalgado} por R${item}{resetar}')
                elif saldo < item:
                    print(f'{vermelho}Você não tem dinheiro suficiente{resetar}')
                elif enroladinho == '1':
                    print(f'{vermelho}Você voltou ao menu{resetar}')
            ...
            pastel = input(f'''{amarelo}Deseja comprar {tipodesalgado} por R$7.99?{resetar} 
                           [0] {verde}Sim{resetar}
                           [1] {vermelho}Não{resetar}: ''')
            if pastel == '0':
                item = 7.99
            ...
            coxinha = input(f'''{amarelo}Deseja comprar {tipodesalgado} por R$4.5?{resetar}
                           [0] {verde}Sim{resetar} 
                           [1] {vermelho}Não:{resetar} ''')
            if coxinha == '0':
                item = 4.50
            ...
            bolinha = input(f'''{amarelo}Deseja comprar {tipodesalgado} por R$2.5?{resetar}
                           [0] {verde}Sim{resetar}
                           [1] {vermelho}Não:{resetar}: ''')
            if bolinha == '0':
                item = 2.50
            ...
            empada = input(f'''{verde}Deseja comprar {tipodesalgado} por R$5.0?{resetar}
                           [0] {verde}Sim{resetar}
                           [1] {vermelho}Não:{resetar}: ''')
            if empada == '0':
                item = 4.99
            ...
            esfiha = input(f'''{amarelo}Deseja comprar {tipodesalgado} por R$6.0?{resetar}
                           [0] {verde}Sim{resetar} 
                           [1] {vermelho}Não:{resetar}: ''')
            if esfiha == '0':
                item = 5.99
        ...
        valor = 200
        saldo += valor #não atribuir var pois da errado
        print(f'{verde}Uma pequena gorjeta seu saldo agora é de R${saldo}{resetar}')
    elif comida == 'carne':
